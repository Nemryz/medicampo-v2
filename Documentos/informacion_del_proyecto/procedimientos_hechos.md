...
El PreFlightCheck en PreFlightCheck.tsx llama a navigator.mediaDevices.getUserMedia solicitando cámara y micrófono. Si falla, reintenta solo con audio. Si falla también el audio, activa el modo isBlocked y muestra instrucciones visuales. Si al menos el audio está disponible, habilita el botón de entrar. Al hacer clic en el botón de entrar, PreFlightCheck detiene todos los tracks del stream activo llamando a stream.getTracks().forEach(t => t.stop()) antes de llamar a onReady(). Este paso es necesario porque si el stream de PreFlightCheck no se libera antes de que LiveKit intente acceder al hardware, el navegador puede bloquear el acceso a la cámara o el micrófono para el segundo solicitante. La prop onReady() cambia el estado isReady a true en el componente padre.

Cuando isReady es verdadero, Videollamada.tsx ejecuta fetchToken que llama al endpoint /api/livekit/token con el roomId de la URL y el nombre del usuario autenticado. Almacena el token en el estado livekitToken. Simultáneamente, obtiene los datos de la cita llamando a /api/appointments/room/:roomId y los almacena en el estado appointment.

El componente LiveKitRoom de @livekit/components-react se usa como elemento raiz del retorno principal del componente. Todos los subcomponentes que usan hooks de LiveKit (IndicadorCalidadRed, EscenarioVideo, ControlesPersonalizados, ChatConsulta) se renderizan dentro de este elemento. Esto es un requisito del SDK: cualquier hook de LiveKit como useRoomContext, useLocalParticipant, useTracks o useChat lanza una excepcion si se invoca fuera del arbol de componentes de LiveKitRoom. LiveKitRoom recibe el token, la serverUrl (la URL wss:// del servidor LiveKit), y las opciones adaptiveStream y dynacast para optimizacion de ancho de banda. Las opciones publishDefaults especifican el codec de video vp9, la supresion de discontinuidades dtx y la redundancia red para mejorar la calidad en conexiones inestables.

El handler onError de LiveKitRoom gestiona los fallos de conexion con un sistema de reintentos limitado. Cuando el SDK lanza un error, el handler incrementa el contador reconnectAttempts y verifica que no supere el maximo de 3 intentos antes de programar un nuevo intento. Si el contador llega al maximo, desactiva el estado isReconnecting para mostrar la pantalla de error definitiva. El contador solo se reinicia en el handler handleRoomConnected, que se ejecuta cuando la sala establece conexion completa, no cuando simplemente se obtiene un nuevo token. Esto evita que los reintentos sean infinitos cuando la conexion falla repetidamente.

El handler onDisconnected registra la desconexion limpia y borra la referencia al objeto Room. El useEffect de limpieza al desmontar el componente llama a roomRef.current.disconnect() para cerrar la conexion de LiveKit activa y luego recorre todos los elementos de video y audio del DOM para detener sus tracks usando track.stop(), liberando la camara y el microfono del sistema operativo.

Dentro de LiveKitRoom se renderizan EscenarioVideo (los videos de los participantes con GridLayout y ParticipantTile), RoomAudioRenderer (el audio de los participantes remotos), ControlesPersonalizados (botones de micrófono, cámara y colgar) y ChatConsulta (el panel de chat).

El panel derecho muestra contenido diferenciado por rol: si el usuario es DOCTOR muestra el formulario de ficha clínica con los campos diagnosis y prescription; si es PATIENT muestra el nombre y especialidad del médico. Al enviar el formulario, handleSaveRecord verifica que appointment.id exista antes de hacer la peticion. Llama a POST /api/clinical/:appointmentId y si la respuesta es exitosa ejecuta navigate(-1) para salir de la sala. Si la respuesta es un error HTTP, lee el cuerpo de la respuesta y muestra el mensaje del servidor con alert. Si hay un error de red, muestra el mensaje de la excepcion con alert. El boton siempre vuelve al estado no-cargando al finalizar la operacion, independientemente del resultado.
