...
Prueba T01-P9: WebSocket de LiveKit fallando por negociacion HTTP/2 en Caddy
Tipo: falla inicial, corrección exitosa
Descripción: Al ingresar a la videollamada desde el navegador del PC, la consola mostraba el error "WebSocket connection to wss://medicampo-rtc.duckdns.org/rtc/v1 failed" seguido de "could not establish signal connection: Encountered websocket error during connection establishment". Los logs del servidor LiveKit no registraban ninguna nueva conexion cuando el usuario intentaba conectarse, lo que indicaba que las solicitudes no llegaban a LiveKit en absoluto.
La verificacion con curl desde el droplet usando el flag --http2 mostro que Caddy negociaba HTTP/2 con el cliente mediante ALPN durante el handshake TLS. Al usar HTTP/2, el navegador no puede enviar los headers Upgrade: websocket y Connection: Upgrade de la forma estandar de HTTP/1.1. Caddy recibia la solicitud de WebSocket en formato HTTP/2 y la enviaba a LiveKit como una solicitud HTTP/2 normal. LiveKit no encontraba ninguna ruta que manejara ese tipo de solicitud y respondia con 404. El resultado era que el WebSocket nunca se establecía y el cliente de LiveKit reportaba fallo de conexion.
Corrección aplicada: Se agregaron dos directivas al Caddyfile. En el bloque global se agrego servers con protocols h1 para desactivar HTTP/2 y HTTP/3 en las conexiones entrantes al servidor. Dentro del bloque reverse_proxy se agrego transport http con versions 1.1 para forzar HTTP/1.1 en la conexion upstream de Caddy hacia LiveKit. Se recargo la configuracion con docker exec caddy-server caddy reload --config /etc/caddy/Caddyfile. Tras la correccion, el curl de verificacion respondio con HTTP/1.1 401 Unauthorized, confirmando que el WebSocket llego a LiveKit correctamente.

Prueba T01-P10: Loop infinito en la pantalla de reconexion del frontend
Tipo: falla inicial, corrección exitosa
Descripción: Cuando el SDK de LiveKit disparaba el evento onError (por fallo de conexion, ICE o cualquier otro error de red), el componente Videollamada.tsx activaba el estado isReconnecting en true y llamaba a fetchToken despues de 3 segundos. La funcion fetchToken obtenia un nuevo token del backend sin problemas, pero no desactivaba el estado isReconnecting al terminar. El componente quedaba mostrando la pantalla "Reconectando... Intento 0 de 3" de forma indefinida porque la variable de estado nunca volvia a false. Adicionalmente, el contador reconnectAttempts se reiniciaba a cero cada vez que fetchToken tenia exito, impidiendo que el maximo de 3 intentos se alcanzara. El ciclo era: onError activa reconexion, fetchToken tiene exito y reinicia el contador, LiveKitRoom vuelve a montar, onError vuelve a disparar, loop sin fin.
Corrección aplicada en frontend/src/components/Videollamada.tsx: La funcion fetchToken llama a setIsReconnecting(false) al obtener el token con exito. El contador reconnectAttempts solo se reinicia en el handler handleRoomConnected, que se ejecuta unicamente cuando la sala establece conexion completa. El handler onError incrementa el contador antes de cada intento y verifica que no supere el maximo definido antes de programar el siguiente reintento. Al llegar al maximo, setIsReconnecting(false) muestra la pantalla de error definitiva con el boton de reintentar.

Prueba T01-P11: Boton "Finalizar Consulta" sin retroalimentacion de error
Tipo: falla inicial, corrección exitosa
Descripción: Al hacer clic en el boton "Finalizar Consulta", el boton mostraba el spinner de carga (isSaving = true). Si la peticion al backend fallaba por cualquier motivo (error de red, appointment sin cargar, respuesta HTTP de error del servidor), el bloque finally desactivaba el spinner pero no existia ningun codigo que mostrara informacion al medico. La sala de videollamada permanecia en pantalla sin navegar a ninguna parte y sin mensaje visible. El medico no podia saber si la ficha se guardo o si fallo. Para salir de la sala debia usar el boton de colgar en lugar del flujo normal de finalizacion de consulta.
Corrección aplicada en frontend/src/components/Videollamada.tsx: Se agrego una verificacion de que appointment.id exista antes de intentar enviar la peticion, con un alert informativo si no existe. Se agrego un bloque que lee el cuerpo de la respuesta cuando el servidor devuelve un codigo de error HTTP y muestra el mensaje exacto del servidor mediante alert. Se agrego un bloque catch alrededor de toda la operacion que captura errores de red y los muestra al medico. El bloque finally siempre desactiva el spinner independientemente del resultado de la operacion.

Prueba T01-P12: Conexion del paciente desde navegador movil
Tipo: pendiente de verificacion
Descripcion: Durante las sesiones de prueba, el paciente que intenta ingresar desde un navegador movil recibe el error "No autorizado, token faltante" o entra en un ciclo de reconexion que no avanza. Se identificaron dos posibles causas. La primera es que el paciente este accediendo directamente a la URL de la sala sin haber iniciado sesion en ese navegador movil especifico. El JWT de autenticacion se almacena en el localStorage de cada navegador de forma independiente. Un navegador movil que nunca hizo login no tiene JWT y el backend rechaza la solicitud de token de LiveKit con estado 401. La segunda posible causa es que el navegador movil tenga un comportamiento especifico con WebRTC o con el handshake de WebSocket que difiera del navegador de escritorio.
Lo que queda por hacer: Ejecutar el flujo completo de prueba desde el telefono movil siguiendo estos pasos en orden. Primero, abrir el navegador del telefono y navegar a la URL principal de la aplicacion. Segundo, iniciar sesion con las credenciales del paciente desde esa pantalla de login. Tercero, una vez en el dashboard del paciente, navegar a la cita confirmada y entrar desde el boton correspondiente sin escribir la URL directamente en la barra de direcciones. Cuarto, abrir la consola de depuracion del navegador movil y registrar los mensajes que aparecen durante el intento de conexion. En Chrome para Android, la consola se accede desde chrome://inspect en el navegador de escritorio con el telefono conectado por USB y la depuracion USB activada. Quinto, comparar los errores que aparecen con los de la tabla de problemas conocidos en la seccion 10 del procedimiento de despliegue.

Prueba T01-P13: Funcionamiento del boton "Finalizar Consulta" tras el fix de manejo de errores
Tipo: pendiente de verificacion
Descripcion: Se aplico un fix al handler handleSaveRecord en Videollamada.tsx que agrega verificacion de appointment.id, manejo de errores HTTP con alert y captura de errores de red. El fix fue subido a produccion pero no se verifico su funcionamiento en el entorno desplegado. No se sabe si el boton navega correctamente al dashboard al guardar con exito, ni si el alert aparece con el mensaje correcto cuando el backend devuelve un error.
Lo que queda por hacer: Entrar a una videollamada como medico desde la produccion desplegada. Escribir un diagnostico en el campo correspondiente. Hacer clic en "Finalizar Consulta". Verificar que la pagina navega al dashboard del medico. Verificar que en el dashboard la cita aparece con estado COMPLETED en la seccion de atenciones recientes. Para probar el caso de error, intentar finalizar la consulta con el campo de diagnostico vacio (el atributo required del formulario deberia prevenir el envio antes de llegar al backend). Si se quiere probar el error del backend directamente, se puede detener temporalmente el servidor backend y verificar que aparece el alert de error de red.
...
Casos de falla encontrados y corregidos: El error 404 al recargar páginas con rutas dinámicas (resuelto con HashRouter). Los permisos de cámara bloqueados sin guía al usuario (resuelto con PreFlightCheck y modo de solo audio). La falla de conexión P2P en redes distintas (resuelto migrando a LiveKit SFU). Los puertos UDP cerrados en el firewall de DigitalOcean (resuelto configurando las reglas de firewall). El certificado SSL faltante en el servidor LiveKit (resuelto configurando Caddy correctamente). El WebSocket de LiveKit fallando por negociacion HTTP/2 en Caddy (resuelto desactivando HTTP/2 con protocols h1 y forzando HTTP/1.1 en el upstream con transport http versions 1.1). El loop infinito en la pantalla de reconexion del frontend (resuelto corrigiendo el orden de reset del contador de intentos y agregando setIsReconnecting(false) en el camino de exito de fetchToken). El boton Finalizar Consulta sin retroalimentacion de error (resuelto agregando manejo de errores HTTP y de red con alert informativo).
